...
class V2Vchannels:              
    ...
    def update_pathloss(self):
        ...
        for i in range(len(self.positions)):
            for j in range(len(self.positions)):
                ...
                if self.PathLoss[i][j] > 5000:
                    print('help!! : ', self.PathLoss[i])
    ...
    def PL_NLosv(self, d_3d):
        ...
        if block_vehicle == 'type1':      #Block Vehicle Height < TX, RX Antenna
            ...
            return self.PL_Los(d_3d) + min(22.2, max(0, np.random.lognormal(5 + max(0, 15 * np.log10(d_3d)-41), 4))) # NLOSv maximum 22.2dB
        else:                            #Block Vehicle Height > TX, RX Antenna
            return self.PL_Los(d_3d) + min(22.2, max(0, np.random.lognormal(9 + max(0, 15 * np.log10(d_3d)-41), 4.5))) # NLOSv maximum 22.2dB
    ...
    def get_path_loss(self, position_A, position_B):
        ...
        d1 = abs(position_A[0] - position_B[0]) + 0.001
        d2 = abs(position_A[1] - position_B[1]) + 0.001
...
Below 6GHz Parameter (완료)
Probability NLOS, LOS(완료) 3GPP 38.901
Pathloss : LOS, NLOS(완료, but 적용) 3GPP 38.901, 3GPP 37.885
Shadowing : log-normal(완료 36.885와 동일)
Fast fading : reyleigh fading(완료 36.885와 동일)
"""

class V2Ichannels: 
    # Simulator of the V2I channels
    def __init__(self, n_Veh, n_RB):
        self.h_bs = 25  # Macro BS의 안테나 높이 In below 6GHz parameters using same carrierfrequency with V2V link
        self.h_ms = 1.6 #3GPP TR 37.885 Antenna 1.6m
        self.Decorrelation_distance = 50        
        self.BS_position = [750/2, 1299/2]    # Suppose the BS is in the center
        self.shadow_std = 4 #3GPP 38.901 for LOS
        self.n_Veh = n_Veh
        self.n_RB = n_RB
        self.update_shadow([])
        self.fc = 5.7 #3GPP TR 37.885 Below 6GHz Parameter
    ...
    def PathLoss_LOS1(self, vehiclePosition):
        
        #차량의 안테나 기준으로 3D 좌표를 저장
        vehiclePositionVector = vehiclePosition + [self.h_ms]
        BaseStationPositionVector = self.BS_position + [self.h_bs]
    ...
    def PathLoss_LOS2(self, vehiclePosition):
        
        #차량의 안테나 기준으로 3D 좌표를 저장
        vehiclePositionVector = vehiclePosition + [self.h_ms]
        BaseStationPositionVector = self.BS_position + [self.h_bs]
        d_3D = self.get_Euclidean_Distance(BaseStationPositionVector, vehiclePositionVector)
        pathLoss = 28.0 + 40 * np.log10(d_3D) + 20 * np.log10(self.fc) - 9 * np.log10((self.d_bp ** 2) + (self.h_bs - self.h_ms) ** 2)
        return pathLoss
    
    def IsPathlossLOS1(self, vehiclePosition):
       #차량의 안테나 기준으로 3D 좌표를 저장
       vehiclePositionVector = vehiclePosition
       BaseStationPositionVector = self.BS_position
    ...
    def PathLoss_NLOS1(self, vehiclePosition):
        
        if self.IsPathlossLOS1(vehiclePosition) == True:
            return max(self.PathLoss_LOS1(vehiclePosition), self.PathLoss_NLOS2(vehiclePosition))
        else:
            return max(self.PathLoss_LOS2(vehiclePosition), self.PathLoss_NLOS2(vehiclePosition))
        
           
    def PathLoss_NLOS2(self, vehiclePosition):
        
        #차량의 안테나 기준으로 3D 좌표를 저장
        vehiclePositionVector = vehiclePosition + [self.h_ms]
        BaseStationPositionVector = self.BS_position + [self.h_bs]
        d_3D = self.get_Euclidean_Distance(BaseStationPositionVector, vehiclePositionVector)
        pathLoss = 13.54 + 39.08 * np.log10(d_3D) + 20 * np.log10(self.fc) - 0.6 * (self.h_ms - 1.5)
        return pathLoss
    
    def IsPathlossNLOS1(self, vehiclePosition):
       #차량의 안테나 기준으로 3D 좌표를 저장
       vehiclePositionVector = vehiclePosition
       BaseStationPositionVector = self.BS_position
       d_2D = self.get_Euclidean_Distance(BaseStationPositionVector, vehiclePositionVector)
    ...
    """Start PathLoss NLOS Function 3GPP 38.901"""
    
    def IsLOS(self, vehiclePosition):
        vehiclePositionVector = vehiclePosition
        baseStationPositionVector = self.BS_position
        d_2D = self.get_Euclidean_Distance(baseStationPositionVector, vehiclePositionVector)
        if d_2D <= 18: # 100% LOS when below 2D 18m
            return True
        else:
            ProbLOS = (18.0 / d_2D) + np.exp(-d_2D/63.0)*(1-18/d_2D) # LOS probability 
            
            if random.uniform(0,1) <= ProbLOS:
                return True
            else:
                return False
       
    """End PathLoss LOS Function 3GPP 38.901""" 
    
    def update_pathloss(self):
        self.PathLoss = np.zeros(len(self.positions))
        for i in range(len(self.positions)):
            if self.IsLOS(self.positions[i]) == True:
                self.shadow_std = 4
                if self.IsPathlossLOS1(self.positions[i]) == True:
                    self.PathLoss[i] = self.PathLoss_LOS1(self.positions[i])
                else:
                    self.PathLoss[i] = self.PathLoss_LOS2(self.positions[i])
            else:
                self.shadow_std = 6
                if self.IsPathlossNLOS1(self.positions[i]) == True:
                    self.PathLoss[i] = self.PathLoss_NLOS1(self.positions[i])
                else:
                    self.PathLoss[i] = self.PathLoss_NLOS2(self.positions[i])
            
            if self.PathLoss[i] > 5000:
                print('help!! : ', self.PathLoss[i])
...
class Environ:
    ...
    def __init__ (self, down_lane, up_lane, left_lane, right_lane, width, height, n_Veh):
        ...
        self.bsAntGain = 8           #기지국 안테나 gain 3GPP 37.885
        self.bsNoiseFigure = 5       #기지국 수신 잡음 지수 3GPP 37.885
        
        self.vehAntGain = 3          #차량 안테나 gain 3GPP 37.885
        self.vehNoiseFigure = 9      #차량 수신 잡음 지수 3GPP 37.885
