...
class Agent(BaseModel):
    ...
    def get_state_external_env(self, agentindex, env, idx):
    # ===============
    #  Get State from the environment
    # =============
        vehicle_number = len(env.vehicles)
        
        #idx번째 차량이 전송하고자하는 v2v link의 resource block의 채널 상태를 보여줌
        V2V_channel = (env.V2V_channels_with_fastfading[idx[0],env.vehicles[idx[0]].destinations[idx[1]],:] - 80)/60
        ...
        for i in range(3):
            for j in range(3):
                if self.training:
                    NeiSelection[self.action_all_with_power_training[env.vehicles[idx[0]].neighbors[i], j, 0 ]] = 1
                else:
                    NeiSelection[self.action_all_with_power[env.vehicles[idx[0]].neighbors[i], j, 0 ]] = 1
        ...
        time_remaining = np.asarray([env.demand[agentindex][idx[0],idx[1]] / env.demand_amount])
        load_remaining = np.asarray([self.env.individual_time_limit[agentindex][idx[0],idx[1]] / env.V2V_limit])
