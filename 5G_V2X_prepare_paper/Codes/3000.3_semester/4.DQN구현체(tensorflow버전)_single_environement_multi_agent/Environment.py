...
class Environ:
    ...
    def __init__ (self, down_lane, up_lane, left_lane, right_lane, width, height, n_Veh, numberOfcompareMode):
        ...
        self.V2V_Interference_all = [np.zeros((self.n_Veh, 3, self.n_RB)) + self.sig2 for _ in list(AgentMode)]
        self.demands = [[] for _ in list(AgentMode)]            # 요구하는 차량의 수 ?
    ...
    def act_for_training_HDH(self, actions, idx):
        ...
        tempaction = actions[:,:,1].reshape(actions[:,:,1].shape[-1] * actions[:,:,1].shape[-2])
        tempaction = np.where(tempaction == 0, 23, tempaction)
        tempaction = np.where(tempaction == 1, 10, tempaction)
        tempaction = np.where(tempaction == 2, 5, tempaction)

        Power_reward = (powerlambda * -((tempaction.mean() + 0.000001) / 23.0))

        #print("time left", time_left)

        #return t
        return t - (self.V2V_limit - time_left)/self.V2V_limit + Power_reward, t_best - (self.V2V_limit - time_left)/self.V2V_limit + Power_reward
...
d
    ...
    def new_random_game(self, n_Veh = 0):
        ...
        self.demand = [self.demand_amount * np.ones((self.n_Veh,3)) for _ in list(AgentMode)]
        self.test_time_count = [10 for _ in list(AgentMode)] 
        self.individual_time_limit = [self.V2V_limit * np.ones((self.n_Veh,3)) for _ in list(AgentMode)]
        self.individual_time_interval = [np.random.exponential(0.05, (self.n_Veh,3)) for _ in list(AgentMode)] 
        self.UnsuccessfulLink = [np.zeros((self.n_Veh,3)) for _ in list(AgentMode)] 
        self.success_transmission = [0 for _ in list(AgentMode)]
        self.failed_transmission = [0 for _ in list(AgentMode)]
        self.activate_links = [np.zeros((self.n_Veh,3), dtype='bool') for _ in list(AgentMode)] 
