...
class Agent(BaseModel):
    ...
    # =============
        ...
        # V2V_channel : #idx번째 차량이 전송하고자하는 v2v link의 resource block의 채널 상태를 보여줌 (포함 하면 local information이 아님.)
        # 근접한 차량이 선택한 리소스 블록 상태
        # 남은 시간
        # 걸린 시간

        #return np.concatenate((V2I_channel, V2V_interference, V2V_channel, NeiSelection, time_remaining, load_remaining)) #V2V Channel을 알고있다고 가정. (state : 82)
        return np.concatenate((V2I_channel, V2V_interference,NeiSelection, time_remaining, load_remaining)) #실제 local information (state : 62)
    ...
    def train(self):  
        ...
        wandb.init(project="V2V Resource Allocation by DQN, local observation (Power)")
        wandb.config["test"] = "DQN Version 0.2"
    ...
    def load_keras_model(self):
        ...
        rootpath = '/home/cnlab1/workspace/MS/5G_V2X_prepare_paper/Codes/3000.3_semester/3.DQN구현체(tensorflow버전)_local_information_nopower/weight/v2i,v2v,qos'
