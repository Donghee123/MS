...
class Environ:
    ...
    # =============
        ...
        V2V_channel = (self.V2V_channels_with_fastfading[idx[0],self.vehicles[idx[0]].destinations[idx[1]],:]).astype(np.int64)  #HDH

        #idx번째 차량이 전송하고자하는 v2i link의 resource block의 채널 상태를 보여줌
        #V2I_channel = (self.V2I_channels_with_fastfading[idx[0], :] - 80)/60 
        V2I_channel = (self.V2I_channels_with_fastfading[idx[0], :]).astype(np.int64)   #HDH
        ...
        V2V_interference = (-self.V2V_Interference_all[idx[0],idx[1],:]).astype(np.int64)  #HDH
    ...
    def act_for_training(self, actions, idx):
        ...
        #return t
        return t - (self.V2V_limit - time_left)/self.V2V_limit, t_best - (self.V2V_limit - time_left)/self.V2V_limit, V2V_rewardlist
