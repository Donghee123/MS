...
class Environ:
 ...
 #               Deficit_list[i,power_idx] = 0 - 1 * np.sum(np.maximum(np.zeros(V2V_Signal_temp.shape), (self.demand - self.individual_time_limit * V2V_Rate_cur * 1500))) # Reward 수정 
                Deficit_list[i,power_idx] = 0 - 1 * np.sum(np.abs(self.demand - self.individual_time_limit * V2V_Rate_cur * 1500))
    ...
    def act_for_training(self, actions, idx):
        ...
        #return t
        return t - (1000 * (self.V2V_limit - time_left)/self.V2V_limit)
