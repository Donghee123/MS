...
class SAC(object):
    def __init__(self, num_inputs, action_space, args, env):
        ...
        hidden_layers = [args.hidden1_size,args.hidden2_size,args.hidden3_size ]
        
        self.critic = QNetwork(num_inputs, action_space.shape[0], hidden_layers).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)
        self.training = True
        self.critic_target = QNetwork(num_inputs, action_space.shape[0], hidden_layers).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space.shape[0], hidden_layers, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.lr)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(num_inputs, action_space.shape[0], hidden_layers, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.lr)
