...
def train_V2(num_iterations, agent, env,  evaluate, args):
    ...
    while episode < totalEpisode:
        ...
        observation = env.get_state(idx = [0,0], isTraining = False, action_all_with_power_training = agent.action_all_with_power_training, action_all_with_power = agent.action_all_with_power)
        agent.reset(deepcopy(observation))
            ...
            for k in range(test_sample):
                ...
                for i in range(len(env.vehicles)):
                    ...
                    for j in sorted_idx:                   
                        ...
                        print('Selected RB, Selected Power : ', [selRBIndex, selPowerdBm])
                        ...
                        agent.observe(deepcopy(reward), deepcopy(observation2), done)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_iter', default=40001, type=int, help='train iters each timestep') # 200000
  ...
  #      args.resume = 'output/{}-run0'.format(args.env)
    ...
    if args.mode == 'train':
        ...
        train_V2(args.train_iter, agent, env, evaluate, 
            args)
    elif args.mode == 'test':
