...
def predict(agent, s_t, ep, test_ep = False, decay_epsilon = True):
    ...
    if np.random.rand() < ep and not test_ep:
        selectRB = np.random.randint(n_RB)
        selectPowerdBm = np.random(0,23.0)
        
        pred_action = np.array([selectRB, selectPowerdBm])
    else:
...
for ind_agent in range(n_veh * n_neighbor):  # initialize agents
    ...
    agent = Agent(DDPGModel, memory_entry_size=len(get_state(env)))
    agents.append(agent)
