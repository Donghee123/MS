...
class Environ:
    ...
    def Compute_Performance_Reward_Batch(self, actions_power, idx):    # add the power dimension to the action selection
        ...
        for i in range(self.n_RB):
            ...
            #현재 리소스블럭을 선택한 indexes들 수집.
            indexes = np.argwhere(actions == i) 
            
            #이전까지 계산된 V2V_Signal 복사
            ...
            for power_idx in range(len(self.V2V_power_dB_List)):
                ...
                V2V_Rate_cur = np.log2(1 + np.divide(V2V_Signal_temp, V2V_Interference_temp + 1e-35))#V2V link의 SINR 계산. 0나누기 방지
                ...
                afterServicedataRemain = self.demand - (self.individual_time_limit * V2V_Rate_cur * 1500)
                ...
                #Deficit_list[i,power_idx] = 0 - 1 * np.sum(\
                                                           #np.maximum( np.zeros(V2V_Signal_temp.shape), (self.demand - (self.individual_time_limit * V2V_Rate_cur * 1500) ) ) \
                                                               #) 
