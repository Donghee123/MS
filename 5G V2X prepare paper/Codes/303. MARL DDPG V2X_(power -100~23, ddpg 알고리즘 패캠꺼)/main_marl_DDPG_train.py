...
def predict(agent, s_t, ou_noise, test_ep = False, decay_epsilon = True, ):

    n_power_levels = len(env.V2V_power_dB_List)
    
    pred_action = agent.get_action(s_t).cpu().numpy() + ou_noise()[0]
    
    pred_action[0][0] = np.clip(pred_action[0][0], 0, 3.9)
    
    pred_action[0][1] = np.clip(pred_action[0][1], -100.0, 23.0)
            ...
            for i_step in range(n_step_per_episode):
                ...
                for i in range(n_veh):
                    for j in range(n_neighbor):
                        ...
                        action = predict(agents[agentIndex], state, ou_noise, epsi) 
                        action = action[0]
                ...
                for i in range(n_veh):
                    for j in range(n_neighbor):
                        ...
                        experience = (torch.tensor(state_old).view(1,33),
                                      torch.tensor(action).view(1,2),
                                      torch.tensor(train_reward_temp).view(1,1),
                                      torch.tensor(state_new).view(1,33),
                                      torch.tensor(1).view(1,1),)
                        
                        memorys[agentIndex].push(experience)
                        
                        # training this agent
                        if len(memorys[agentIndex]) > batch_size:
                                   #train agent
                            sampled_exps = memory.sample(batch_size)
                            sampled_exps = prepare_training_inputs(sampled_exps, device = DEVICE)
                            agents[agentIndex].update(*sampled_exps)
                               
                            #update target networks
                            #OUProcess에 의해 noisy 한 환경이 생김.
                            #soft_update를 통해 noisy 한 환경에서 tau값을 조절하며 업데이트함
                            soft_update(agents[agentIndex].actor, agents[agentIndex].actor_target, tau)
                            soft_update(agents[agentIndex].critic, agents[agentIndex].critic_target, tau)
