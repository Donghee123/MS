...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        action[0][0] = action[0][0].clamp(0, 3.9)
        action[0][1] = action[0][1].clamp(-100.0, 23.0)
        return action
...
class DDPG(nn.Module):
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        with torch.no_grad():
            ...
            critic_target = r + self.gamma * self.critic_target(ns, self.actor_target(ns))
    ...
    def save_model(self, model_path, saveFileName):
        ...
        saveActorFileName = savePath + '/actor_' + saveFileName + '.pkl'
        saveCriticFileName = savePath + '/critic_' + saveFileName + '.pkl'
