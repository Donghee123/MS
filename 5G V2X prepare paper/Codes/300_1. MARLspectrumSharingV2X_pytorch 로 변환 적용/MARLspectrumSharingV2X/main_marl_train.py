...
if IS_TRAIN:
        ...
        for i_step in range(n_step_per_episode):
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    state = to_tensor(state, size=(1, n_input))
                    
                    state_old_all.append(state)
                    
                    action = predict(agents[i*n_neighbor+j], state, epsi)
                    
                    action_all.append(action)
                    
                    all_select_rb[time_step] = action % n_RB
                    all_select_power[time_step] = int(np.floor(action / n_RB)) #23 15 5 -100
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    state_new = to_tensor(state_new, size=(1, n_input))
                    
                    #1개의 경험 튜플
                    experience = (state_old, 
                                  torch.tensor(action).view(1,1),
                                  torch.tensor(train_reward / 100.0).view(1,1),#r값을 스케일링함
                                  torch.tensor(state_new).view(1,n_input),
                                  torch.tensor(False).view(1,1))
                    
                    #각 agent들의 memory에 저장
                    memorys[i * n_neighbor + j].push(experience)  # add entry to this agent's memory
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    if len(memory) >= sampling_only_util:
                        sampled_exps = memorys[i*n_neighbor+j].sample(batch_size)
                        sampled_exps = prepare_training_inputs(sampled_exps)    
                        agents[i*n_neighbor+j].update(*sampled_exps)
                    ...
                    if i_step % target_update_interval == 0:
                        ...
                        agents[i*n_neighbor+j].qnet_target.load_state_dict( agents[i*n_neighbor+j].qnet.state_dict() )
    ...
    for i in range(n_veh):
        for j in range(n_neighbor):
            ...
            save_models(agents[i*n_neighbor+j], model_path)
...
if IS_TEST:
    ...
    mulbytes = [1,2,3,4,5,6]
    
    for mulbytes in mulbytes:
        
        env.demand_size = int((4 * 190 + 300) * 8 * mulbytes)
