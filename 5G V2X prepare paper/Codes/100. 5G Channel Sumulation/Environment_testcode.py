...
from Environment_5G import Environ as Environ
...
def Show_V2I_ALL_lossgraph(ax, Env, tick, color):
...
def Show_V2I_Pathlossgraph(ax, Env, tick, color):
...
for i in range(len(figs)):
    ...
    axs.append(figs[i].add_subplot(1,2,2))
...
IsAdaptneighbors = False # True : 형성된 V2V Link를 보여줌, Fals : 형성된 V2I Link를 보여줌
color = ['b','g','r']
...
for i in range(teststep):
    ...
    if i % capture_term == 0:
        ...
        #Show_V2I_Pathlossgraph(axs[count+1], Env, tick, 'r')
        Show_V2I_ALL_lossgraph(axs[count+1], Env, tick, 'r')     
        count+=2
...
v2i_NumpyArray = []
v2i_NumpyArray.append(v2ilink_allfading)
v2i_NumpyArray = np.array(v2i_NumpyArray).transpose()

SaveNumpyArrayDataToCSV('neigbor.csv', neibors_NumpyArray)
SaveNumpyArrayDataToCSV('V2I.csv', v2i_NumpyArray)
