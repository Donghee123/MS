...
class Environ:
    ...
    def Compute_Performance_Reward_fast_fading_with_power_asyn(self, actions_power):   # revising based on the fast fading part
        ...
        for i in range(len(power_selection)):
            for j in range(len(power_selection[i])):
                power_selection[i][j] = self.find_nearest_arg(self.V2V_power_dB_List, power_selection[i][j])
        
        power_selection = np.array(power_selection, dtype=np.int)
    ...
    def Compute_Interference(self, actions):
        ...
        if len(actions.shape) == 3:
            ...
            channel_selection = np.array(channel_selection, dtype=np.int)
    ...
    def act_for_training(self, actions, idx):
        ...
        power_selection = action_temp[:,:,1] 
        for i in range(len(power_selection)):
            for j in range(len(power_selection[i])):
                power_selection[i][j] = self.find_nearest_arg(self.V2V_power_dB_List, power_selection[i][j])
                
        action_temp = np.array(action_temp, dtype=np.int)
