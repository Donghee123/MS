...
class RetroEnv(gym.Env):
    ...
    def step(self, a):
        ...
        if self.make_video:
            ...
            print('create folder video_frames)
            os.system('mkdir video_frames)
            cv2.imwrite('video_frames/'+baseline_str + self.gamename + '-' + str(self.n) + '.png',cv2.cvtColor(ob, cv2.COLOR_RGB2BGR)) #HDH
        self.n +=1
    ...
    def reset(self):
        if self.audio_clip:
            ...
            if self.make_video:
                ...
                cmd1 = 'ffmpeg -y -r 60 -f image2 -i video_frames'+str(self.n) +'/' + baseline_str + self.gamename + '-%d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p ' + baseline_str + self.gamename + '_noaudio.mp4 -hide_banner -loglevel panic'#HDH
                ...
                os.system(cmd)
                #os.system('rm video_frames/' + baseline_str + self.gamename+'*.png') #HDH
                print('saved video to ',baseline_str + self.gamename + '-' + str(self.n)+'test'+ '.mp4')
                sys.exit(0)
