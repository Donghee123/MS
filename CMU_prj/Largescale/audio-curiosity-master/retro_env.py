...
class RetroEnv(gym.Env):
    ...
    def __init__(self, game, state=retro.State.DEFAULT, scenario=None, info=None, use_restricted_actions=retro.Actions.FILTERED,
        ...
        self.saveLogPath = f'{self.now.hour}-{self.now.minute}_{self.now.date()}_{self.now.microsecond}'
        os.system(f'mkdir {self.saveLogPath}')
        os.system(f'mkdir {self.saveLogPath}/audioimage')
        os.system(f'mkdir {self.saveLogPath}/audio')
        os.system(f'mkdir {self.saveLogPath}/non_audioimage')
    ...
    def step(self, a): #HDH
        ...
        if self.naudio_samples is not None:
            ...
            if np.max(info['audio']) == 0:
                if random.random() >= 0.8:
                    img = Image.fromarray(np.array(ob)) #HDH
                    img.save(f'{self.saveLogPath}/audioimage/image_{self.stepCount}.png')#HDH
                    np.save(f'{self.saveLogPath}/audio/audio_{self.stepCount}',np.array(info['audio']))      #HDH      
            else:
                    img = Image.fromarray(np.array(ob)) #HDH
                    img.save(f'{self.saveLogPath}/audioimage/image_{self.stepCount}.png')#HDH
                    np.save(f'{self.saveLogPath}/audio/audio_{self.stepCount}',np.array(info['audio']))      #HDH   
            ...
            img = Image.fromarray(np.array(ob)) #HDH
            img.save(f'{self.saveLogPath}/non_audioimage/image_{self.stepCount}.png')#HDH

        self.stepCount += 1

        if self.make_video:
            baseline_str = 'b-' if self.is_baseline else ''
            cv2.imwrite(f'video_frames/'+baseline_str + self.gamename + '-' + str(self.n) + '.png',cv2.cvtColor(ob, cv2.COLOR_RGB2BGR))
    ...
    def reset(self):
        ...
        if self.audio_clip:
            if self.make_video:
                ...
                #cmd1 = f'ffmpeg -y -r 60 -f image2 -i video_frames{self.envNumber}/' + baseline_str + self.gamename + '-%d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p ' + baseline_str + self.gamename + '_noaudio.mp4 -hide_banner -loglevel panic'#HDH
                ...
                os.system(cmd)
                os.system(f'rm video_frames/' + baseline_str + self.gamename+'*.png') #HDH
