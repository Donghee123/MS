...
def plot_run(paths, x='tcount', ys='eprew', label='', assign_colors=True, params=''):
    for y in ys.split(','):
        ...
        saveX = np.array(nframes)
        saveY = np.array(mean_run)
        
        saveData = np.array([[saveX[i], saveY[i]] for i in range(len(saveX))])

        df = pd.DataFrame(saveData)

        df.columns=['t_step', 'reward_mean']

        

        df.to_csv(f'{os.path.basename(os.path.normpath(paths[0]))}_data.csv', index=False)
...
def create_params(path):
    ...
    if args.mean:
        ...
        if 'disagreement' in path or 'openai' in path:
            ...
            print('use get_disagreement_params')
        if 'joint' in path:
            params += '_joint'
    return params
...
if __name__ == '__main__':
    ...
    parser.add_argument('--paths', type=str, default = './logs/extrinsic_intrinsic_reward_method')
