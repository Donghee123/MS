...
class Agent(BaseModel):
    ...
    def observe(self, prestate, state, reward, action):
        ...
        if self.step > 0:
            if self.step % 50 == 0:
                #print('Training')
                totalLoss = self.q_learning_mini_batch()            # training a mini batch
            ...
            for k in range(1):      
                ...
                for i in range(len(self.env.vehicles)):
                    ...
                    for j in range(3): 
                        ...
                        oneStepReward += reward_train
                        one_step_reward_value = one_step_reward_value + reward_train
                ...
                logs.append((": 1 step Cumulative Reward : " + str(one_step_reward_value) + '-- Target Reward : ' + str(ep_target_reward) + '-- Diff DQN vs target : ' + str(ep_target_reward - one_step_reward_value)))
                wandb.log({"DQN_reward": one_step_reward_value, "target_reward" : ep_target_reward, "Diff DQN vs target" : ep_target_reward - one_step_reward_value})
                

                ep_target_reward = 0.
                one_step_reward_value = 0.

            oneStepRewards.append(oneStepReward)
    ...
    def playwithKeras(self, n_step = 100, n_episode = 100, test_ep = None, render = False, random_choice = False, use_async = True):
        ...
        for game_idx in range(number_of_game):
            ...
            print('Mean of the patital V2V Avg rate is that ', np.mean(V2V_AvGRate_list[0:game_idx] ))
            print('Mean of Fail percent is that ',percent, np.mean(Fail_percent_list[0:game_idx]))
            print('Mean of Selected Power (mW)',np.mean(power_list[0:game_idx]))
            print('Mean of Selected var Power (mW)',np.mean(varpower_list[0:game_idx]))
            print('Mean of Selected std Power (mW)',np.mean(stdpower_list[0:game_idx]))
        ...
        print('Mean of Fail percent is that ', np.mean(Fail_percent_list))
        print('Mean of Selected Power (mW)',np.mean(power_list))
        print('Mean of Selected var Power (mW)',np.mean(varpower_list))
        print('Mean of Selected std Power (mW)',np.mean(stdpower_list))
        # print('Test Reward is ', np.mean(test_result))
